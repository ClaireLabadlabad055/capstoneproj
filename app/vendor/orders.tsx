...
const CollapsibleOrderCard = ({ order, onStatusChange }: { order: Order; onStatusChange: (id: string, status: string) => void }) => {
  ...
  const batchInfo = order.pickupBatch || order.pickup_batch || 'Batch 1 (10:00 AM - 11:00 AM)';
  const landmarkInfo = order.pickupLandmark || order.pickup_landmark || 'Public Market Entrance';
    ...
    <View style={styles.orderCard}>
      ...
      <View style={styles.metaTagsRow}>
        <View style={styles.metaBadge}>
          <Feather name="clock" size={12} color="#C2410C" style={{ marginRight: 4 }} />
          <Text style={styles.metaBadgeText}>{batchInfo}</Text>
        </View>
        <View style={styles.metaBadge}>
          <Ionicons name="location-outline" size={12} color="#C2410C" style={{ marginRight: 4 }} />
          <Text style={styles.metaBadgeText} numberOfLines={1}>{landmarkInfo}</Text>
        </View>
      </View>
...
export default function VendorDashboard() {
  ...
  // Group active orders by Checkout Mockup Batches, then by Pickup Location Landmarks
  const groupedActiveOrders = useMemo(() => {
    const map: { [batch: string]: { [landmark: string]: Order[] } } = {};
    
    activeOrders.forEach(order => {
      const batch = order.pickupBatch || order.pickup_batch || 'Batch 1 (10:00 AM - 11:00 AM)';
      const landmark = order.pickupLandmark || order.pickup_landmark || 'Public Market Entrance';

      if (!map[batch]) {
        map[batch] = {};
      }
      if (!map[batch][landmark]) {
        map[batch][landmark] = [];
      }
      map[batch][landmark].push(order);
    });
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.toolGrid}>
          ...
          {/* Added separate Orders tool card right beside Messages */}
          <TouchableOpacity style={styles.toolItem} activeOpacity={0.9} onPress={() => router.push('/vendor/history')}>
            <View style={styles.toolIcon}>
              <Feather name="package" size={22} color="#C2410C" />
            </View>
            <Text style={styles.toolText}>Orders</Text>
          </TouchableOpacity>
        </View>

        <Text style={styles.sectionTitle}>Live Queue (Grouped by Checkout Batches & Locations)</Text>
        {activeOrders.length > 0 ? (
          Object.entries(groupedActiveOrders).map(([batchName, landmarks]) => (
            <View key={batchName} style={styles.batchGroupContainer}>
              ...
              {/* Landmarks under this Batch */}
              {Object.entries(landmarks).map(([landmarkName, landmarkOrders]) => (
                <View key={landmarkName} style={styles.landmarkGroupContainer}>
                  ...
                  {/* Orders under this Landmark */}
                  {landmarkOrders.map((order) => (
                    <CollapsibleOrderCard key={order.id} order={order} onStatusChange={handleStatusTransition} />
                  ))}
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  batchGroupHeaderText: {
    fontSize: 15,
    fontWeight: '900',
    color: '#C2410C',
    letterSpacing: -0.2,
  },
  landmarkGroupContainer: {
  ...
  landmarkGroupHeaderText: {
    fontSize: 13,
    fontWeight: '800',
    color: '#64748B',
  },

  orderCard: { backgroundColor: '#FFFFFF', borderRadius: 18, padding: 16, marginBottom: 10, borderWidth: 1, borderColor: '#F1F5F9', shadowColor: '#C2410C', shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 },
  orderCardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 },
  orderID: { fontWeight: '900', color: '#1E293B', fontSize: 14, letterSpacing: -0.3 },
  customerSub: { color: '#64748B', fontSize: 12, marginTop: 2, fontWeight: '600' },
  metaTagsRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 12 },
  metaBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8FAFC', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, borderWidth: 1, borderColor: '#E2E8F0' },
  ...
  iconActionBtn: { backgroundColor: '#FFEDD5', padding: 6, borderRadius: 10, justifyContent: 'center', alignItems: 'center' },
  itemSummary: { padding: 12, backgroundColor: '#F8FAFC', borderRadius: 12, marginBottom: 12, borderWidth: 1, borderColor: '#E2E8F0' },
  summaryText: { fontSize: 12, fontWeight: '700', color: '#1E293B', marginBottom: 2 },
  summaryQty: { color: '#C2410C', fontWeight: '900' },
  moreItemsText: { fontSize: 10, color: '#C2410C', marginTop: 4, fontWeight: '800' },
  orderFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  orderTotalText: { fontSize: 16, fontWeight: '900', color: '#1E293B', letterSpacing: -0.3 },
  actionBtnGrad: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 12, shadowColor: '#C2410C', shadowOpacity: 0.2, shadowRadius: 4, elevation: 2 },
